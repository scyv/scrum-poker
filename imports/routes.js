...
Router.route("/session/:sessionId", function () {
    if (isLoggedIn()) {
        const sessionId = this.params.sessionId;
        this.wait(Meteor.subscribe(COLLECTIONS.SESSIONS, sessionId, () => {
...
Router.route("/session/:sessionId/story/:storyId", function () {
    if (isLoggedIn()) {
        const sessionId = this.params.sessionId;
        const storyId = this.params.storyId;
        this.wait(Meteor.subscribe(COLLECTIONS.SESSIONS, sessionId, () => {
            Session.set(SessionProps.SELECTED_SESSION, sessionId);
            this.wait(Meteor.subscribe(COLLECTIONS.STORIES, sessionId, () => {
